...
**天才猫 R2 智能云端网盘系统** 是一款专为 Cloudflare R2 设计的高颜值、全功能、轻量级私有云盘系统。默认搭载 **macOS 26 沉浸式云端桌面操作系统** 与 **Apple VisionOS 空间流光展厅** 双模式，深度结合 Vue 3 响应式架构，零编译步骤（Zero Build Step），无需服务器即可在全球边缘节点极速分发。

---

## ✨ 核心全功能矩阵

### 0. 🍏 沉浸式 macOS 26 云端桌面操作系统模式 (默认)
* ** 顶部系统菜单栏 (Top Menu Bar)**：集成 Apple 标志系统下拉菜单（关于系统、系统偏好设置、模式切换、锁屏）、Finder 操作菜单、当前存储桶指示器、☀️/🌙 主题切换、⌘K Spotlight 聚焦搜索、iOS 26 空间控制中心与实时动态时钟；
* **🌊 4K 动态壁纸与桌面工作区**：预设 Sequoia 杉树晨光、Sonoma 极光流彩、Big Sur 晨曦群山等多款 4K 动态壁纸，桌面放置系统图标，支持鼠标半透明橡皮筋框选；
* **⛵ 物理级动效 Dock 栏**：高拟真贝塞尔曲线波浪物理放大（Magnification），点击 App 呈现轻快弹跳（Bounce），底部常驻运行状态白光微指示灯；
* **🪟 多窗口管理引擎 (Window Manager)**：
  * 窗口标配 🔴 红色关闭、🟡 黄色最小化（收缩至 Dock）、🟢 绿色全屏/最大化交通灯；
  * 窗口支持在桌面上**自由拖拽位置、边缘 8 方向拉伸缩放、点击自动提升最高层级**，支持多窗口并行多任务；
* **📂 3 栏式 Finder 访达窗口**：左侧提供【收藏夹 / 分类标签 / 存储桶位置】导航树，顶部工具栏支持图标/列表/新建/上传快捷控制；
* **🎛️ iOS 26 控制中心 & ⌘K Spotlight 聚焦搜索**：右上角滑出毛玻璃控制中心（存储用量、主题调节、全屏开关、壁纸切换），屏幕中央聚焦即时全局检索并直接启动 App 或直达目录。
