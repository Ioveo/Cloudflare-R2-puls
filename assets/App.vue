<template>
<main class="drive-shell" @click="closeContext" @contextmenu.prevent="openContext(null, $event)" @dragenter.prevent @dragover.prevent @drop.prevent="onDrop">
    <header class="topbar">
...
export default {
  ...
  created() { this.fetchStorages(); window.addEventListener("popstate", () => { const url = new URL(window.location); this.cwd = url.searchParams.get("p") || ""; this.storageId = url.searchParams.get("storage") || "default"; }); }, components: { Menu, MimeIcon, UploadPopup, UploadProgress, ContextMenu, PromptDialog },
};
</script>
