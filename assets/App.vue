...
export default {
  ...
  methods: {
    ...
    openFile(file) { if (this.isImage(file)) { const index = this.imageItems.findIndex((item) => item.file.key === file.key); if (index !== -1) { this.lightbox.index = index; this.lightbox.visible = true; return; } } this.preview(this.rawPath(file.key)); },
  ...
  created() { this.fetchStorages(); window.addEventListener("popstate", () => { const url = new URL(window.location); this.cwd = url.searchParams.get("p") || ""; this.storageId = url.searchParams.get("storage") || "default"; }); }, components: { Menu, MimeIcon, UploadPopup, UploadProgress, ContextMenu, PromptDialog, LightboxModal },
};
</script>
